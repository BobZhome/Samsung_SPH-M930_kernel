...
static int msm_pm_power_collapsed_status = -1;
...
//chief.sleep.debug
#define MSM_PM_DEBUG_PRINT_SLEEP_INFO_ALWAYS() \
	do { \
			smsm_print_sleep_info(msm_pm_smem_data->sleep_time, \
				msm_pm_smem_data->resources_used, \
				msm_pm_smem_data->irq_mask, \
...
static int msm_pm_boot_done = 0;
module_param_named(
	boot_done, msm_pm_boot_done,
	int, S_IRUGO | S_IWUSR | S_IWGRP
);
#endif
	...
	(bool from_idle, uint32_t sleep_delay, uint32_t sleep_limit)
{
	...
	KFUNC_TRACE("pc_enter", from_idle);

	if(!from_idle) vreg_print_info("enabled"); //chief.sleep.debug
	...
	if(!from_idle) MSM_PM_DEBUG_PRINT_SLEEP_INFO_ALWAYS();
	...
	if(!from_idle) {
		msm_pm_power_collapsed_status = MSM_PM_SLEEP_MODE_POWER_COLLAPSE_SUSPEND;
//		printk(KERN_ERR "+=+=+=+= POWER COLLAPSE ENTER +=+=+=+= \n"); //chief.sleep.debug
	} else
		msm_pm_power_collapsed_status = MSM_PM_SLEEP_MODE_POWER_COLLAPSE;

	collapsed = msm_pm_collapse();
//	if(!from_idle) printk(KERN_ERR "+=+=+=+= POWER COLLAPSE EXIT (collapsed=%d)+=+=+=+= \n", collapsed);//chief.sleep.debug
	msm_pm_power_collapsed_status = -1;
	...
	if(!from_idle) {
		...
		printk(KERN_ERR "@@      irq_mask : 0x%08x\n", msm_pm_smem_data->irq_mask);
		printk(KERN_ERR "@@      rpc_prog : 0x%08x\n", msm_pm_smem_data->rpc_prog);
		printk(KERN_ERR "@@      rpc_proc : 0x%08x\n", msm_pm_smem_data->rpc_proc);
	} else {
		static int wakeup_reason_cnt[32] = {0, };
		static const char * str_wakeup_reason[32] = {"SMD", "INT", "GPIO", "TIMER", "ALARM", "RESET", "OTHER", "REMOTE", "DPRAM"};
		static int cnt = 0;
		int i;

		for(i=0;i<32;i++) {
			if((msm_pm_smem_data->wakeup_reason>>i) & 0x1)
				wakeup_reason_cnt[i]++;
		}
		if(++cnt >= 500) {
			cnt = 0;
			printk(KERN_ERR "@@ IDLE wakeup stat \n");
			for(i=0;i<32;i++) {
				if(wakeup_reason_cnt[i]>0)
					printk(KERN_ERR "@@  %8s=%3d\n", str_wakeup_reason[i], wakeup_reason_cnt[i]);
				wakeup_reason_cnt[i] = 0;
			}
		}
	}
#endif
...
static int msm_pm_power_collapse_standalone(void)
{
	...
	KFUNC_TRACE("pc_standalone_enter", 0);
	...
	l2x0_suspend();
#endif
	msm_pm_power_collapsed_status = MSM_PM_SLEEP_MODE_POWER_COLLAPSE_STANDALONE;
	collapsed = msm_pm_collapse();
	msm_pm_power_collapsed_status = -1;
#ifdef CONFIG_CACHE_L2X0
	l2x0_resume(collapsed);
	...
	KFUNC_TRACE("pc_standalone_exit", 0);
...
#ifdef CONFIG_MACH_CHIEF
	if(!msm_pm_boot_done) {
		allow[MSM_PM_SLEEP_MODE_RAMP_DOWN_AND_WAIT_FOR_INTERRUPT] = false;
		allow[MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT] = false;
		allow[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_NO_XO_SHUTDOWN] = false;
		allow[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_STANDALONE] = false;
	}
#endif
...
extern int yaffs_set_final_checkpoint(unsigned int);
#endif
...
static void msm_pm_power_off(void)
{
	...
	pr_info("[%s] line %d\n",__func__, __LINE__);
#if defined(CONFIG_MACH_CHIEF)
	yaffs_set_final_checkpoint(0xf07ced);
	sys_sync();
	pr_info("[%s] line %d\n",__func__, __LINE__);
#endif
        if( fsa9480_get_charger_status() ) {
                p_smem_vendor1 = (samsung_vendor1_id *)smem_alloc(SMEM_ID_VENDOR1, sizeof(samsung_vendor1_id));
                if(p_smem_vendor1) {
                        restart_reason = 0;
                        p_smem_vendor1->off_charging_off = 0x22;
                }
...
extern size_t multi_download_mode;
static void msm_pm_restart(char str, const char *cmd)
{
	...
	if(multi_download_mode == 0xabcd1234)
		restart_reason = multi_download_mode;
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if ((code == SYS_RESTART) && _cmd) {
		...
		} else if (!strcmp(cmd, "multidownload")) {
			restart_reason = 0xabcd1234;
