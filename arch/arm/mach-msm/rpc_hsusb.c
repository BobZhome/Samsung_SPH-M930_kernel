...
#define ONCRPC_CHG_USB_CHARGER_DISCONNECTED_PROC 8 
#define ONCRPC_CHG_USB_I_IS_AVAILABLE_PROC 9 
#define ONCRPC_CHG_USB_I_IS_NOT_AVAILABLE_PROC 10 
...
static int msm_chg_init_rpc(unsigned long vers)
{
	if (((vers & RPC_VERSION_MAJOR_MASK) == 0x00010000) ||
		...
		chg_rpc_ids.chg_usb_charger_connected_proc 	= ONCRPC_CHG_USB_CHARGER_CONNECTED_PROC;	//7;
		chg_rpc_ids.chg_usb_charger_disconnected_proc 	= ONCRPC_CHG_USB_CHARGER_DISCONNECTED_PROC;	//8;
		chg_rpc_ids.chg_usb_i_is_available_proc 	= ONCRPC_CHG_USB_I_IS_AVAILABLE_PROC;	//9;
		chg_rpc_ids.chg_usb_i_is_not_available_proc 	= ONCRPC_CHG_USB_I_IS_NOT_AVAILABLE_PROC;	//10;
		return 0;
	} else
		return -ENODATA;
}
