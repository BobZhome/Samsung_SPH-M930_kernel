...
#define YAFFS_FINAL_CHECKPOINT_MAGIC1 ((unsigned int)0xf07ced)
#define YAFFS_FINAL_CHECKPOINT_MAGIC2 ((unsigned int)('C'<<0|'K'<<8|'P'<<16|'T'<<24))
unsigned int yaffs_final_checkpoint_magic1 = 0;
unsigned int yaffs_final_checkpoint_magic2 = 0;

int yaffs_set_final_checkpoint(unsigned int confirm)
{
	if(confirm == YAFFS_FINAL_CHECKPOINT_MAGIC1) {
		yaffs_final_checkpoint_magic1=YAFFS_FINAL_CHECKPOINT_MAGIC1;
		yaffs_final_checkpoint_magic2=YAFFS_FINAL_CHECKPOINT_MAGIC2;
		pr_info("\t@yaffs!! set for final checkpoint!!\n");
		return 0;
	}
...
bool yaffs_is_final_checkpoint(void)
{
	return ((yaffs_final_checkpoint_magic1==YAFFS_FINAL_CHECKPOINT_MAGIC1)&&
		(yaffs_final_checkpoint_magic2==YAFFS_FINAL_CHECKPOINT_MAGIC2));
}
...
#endif
{
	...
	if(unlikely(yaffs_is_final_checkpoint()==1)) {
		pr_info("\t@yaffs!!! checkpointed for power off. skip %s\n", __func__);
		return -EBUSY;
	}
...
static int yaffs_get_num_badblocks(yaffs_Device *dev)
{
	int blk;
...
static void yaffs_FlushSuperBlock(struct super_block *sb, int do_checkpoint)
{
	...
	pr_info("\t@yaffs [%s] %s, do_checkpoint=%d\n", __func__, sb->s_id, do_checkpoint);
				...
				int request_checkpoint)
{
	...
	pr_info("\t@yaffs yaffs_do_sync_fs : %s\n", sb->s_id);
	...
	if(unlikely(yaffs_is_final_checkpoint()==1)) {
		pr_info("\t@yaffs forcing flush super : %s checkpointed:%d dirty:%d\n", 
			sb->s_id, dev->isCheckpointed, sb->s_dirt);
		do_checkpoint = 1;
	}
...
static void yaffs_put_super(struct super_block *sb)
{
	...
	pr_info("\t@yaffs yaffs_put_super : %s\n", sb->s_id);
...
#if defined (CONFIG_MACH_CHIEF)
static struct proc_dir_entry *bbsize_proc_entry;
static struct proc_dir_entry *lastchkpt_proc_entry;
#endif
...
#if defined(CONFIG_MACH_CHIEF)
	buf += sprintf(buf, "nBadBlocks....... %u\n", yaffs_get_num_badblocks(dev));
#endif
	return buf;
}
...
#if defined (CONFIG_MACH_CHIEF)
static int yaffs_bbsize_proc_read(char *page,
				char **start,
				off_t offset, int count, int *eof, void *data)
{
	int nbb;
	...
	ylist_for_each(item, &yaffs_context_list) {
		...
		nbb = yaffs_get_num_badblocks(dev);
		buf += sprintf(buf,"%s:%d\n", dev->param.name, nbb*4096*64);
	}
...
static int yaffs_finalchkpt_proc_read(char *page,
				char **start,
				off_t offset, int count, int *eof, void *data)
{
	char *buf = page;

	buf += sprintf(buf,"%s\n", yaffs_is_final_checkpoint()?"Final Checkpointed!\nyaffs file system wouldn't accept modifications\n":"No\n");
	
	return buf - page < count ? buf - page : count;
}

#include <linux/syscalls.h> /* sys_sync */
static int yaffs_finalchkpt_proc_write(struct file *file, const char *buf,
					 unsigned long count, void *data)
{
	if(!strncmp(buf, "checkpoint", 10)) {
		yaffs_set_final_checkpoint(YAFFS_FINAL_CHECKPOINT_MAGIC1);
		sys_sync();
	}
	return count;
}
...
#if defined (CONFIG_MACH_CHIEF)
	bbsize_proc_entry = create_proc_entry("yaffs_bbsize",
					       S_IRUGO | S_IFREG,
					       YPROC_ROOT);

	if (bbsize_proc_entry) {
		bbsize_proc_entry->write_proc = NULL;
		bbsize_proc_entry->read_proc = yaffs_bbsize_proc_read;
		bbsize_proc_entry->data = NULL;
	} else
		return -ENOMEM;

	lastchkpt_proc_entry = create_proc_entry("yaffs_finalchkpt",
					       S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP,
					       YPROC_ROOT);
	#define AID_SYSTEM         1000 
	lastchkpt_proc_entry->uid = AID_SYSTEM;
	lastchkpt_proc_entry->gid = AID_SYSTEM;
	if (lastchkpt_proc_entry) {
		lastchkpt_proc_entry->write_proc = yaffs_finalchkpt_proc_write;
		lastchkpt_proc_entry->read_proc = yaffs_finalchkpt_proc_read;
		lastchkpt_proc_entry->data = NULL;
	} else
		return -ENOMEM;
#endif
