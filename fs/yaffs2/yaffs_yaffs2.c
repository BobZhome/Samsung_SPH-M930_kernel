...
static int yaffs2_ReadCheckpointValidityMarker(yaffs_Device *dev, int head)
{
	...
	pr_err("\t@yaffs [%s] checkpointread result %d\n", __func__, ok);
...
static int yaffs2_WriteCheckpointData(yaffs_Device *dev)
{
	...
	if (!yaffs2_CheckpointRequired(dev)) {
		...
		ok = 0;
//		pr_info("\t@yaffs skipping checkpoint write ok=%d\n", ok);
	}
...
//		pr_info("\t@yaffs checkpoint validity ok=%d\n", ok);
	}
	if (ok) {
		T(YAFFS_TRACE_CHECKPOINT, (TSTR("write checkpoint device" TENDSTR)));
		ok = yaffs2_WriteCheckpointDevice(dev);
//		pr_info("\t@yaffs checkpoint device ok=%d\n", ok);
	}
	if (ok) {
		T(YAFFS_TRACE_CHECKPOINT, (TSTR("write checkpoint objects" TENDSTR)));
		ok = yaffs2_WriteCheckpointObjects(dev);
//		pr_info("\t@yaffs objects write ok=%d\n", ok);
	}
	if (ok) {
		T(YAFFS_TRACE_CHECKPOINT, (TSTR("write checkpoint validity" TENDSTR)));
...
//		pr_info("\t@yaffs checkpoint sum ok=%d\n", ok);
	}
	if (!yaffs2_CheckpointClose(dev)) {
		ok = 0;
//		pr_info("\t@yaffs checkpoint close ok=%d", ok);
	}
	...
	pr_info("\t@yaffs checkpoint final result ok=%d", ok);
...
//		pr_info("\t@yaffs skipping checkpoint read\n");
		ok = 0;
	}
...
//		pr_info("\t@yaffs checkpoint objects ok=%d\n", ok);
	}
	if (ok) {
		T(YAFFS_TRACE_CHECKPOINT, (TSTR("read checkpoint validity" TENDSTR)));
...
//		pr_info("\t@yaffs checkpoint checksum ok=%d\n", ok);
	}

	if (!yaffs2_CheckpointClose(dev))
		ok = 0;
	
//	pr_err("\t@yaffs [%s] yaffs2_CheckpointClose result %d\n", __func__, ok);
	...
	pr_err("\t@yaffs [%s] yaffs2_Checkpoint final result %d\n", __func__, ok);
...
int yaffs_CheckpointSave(yaffs_Device *dev)
{
	...
	pr_info("\t@yaffs save entry: isCheckpointed %d\n", dev->isCheckpointed);
	...
	T(YAFFS_TRACE_ALWAYS, (TSTR("\t@yaffssave exit: isCheckpointed %d"TENDSTR), dev->isCheckpointed));
