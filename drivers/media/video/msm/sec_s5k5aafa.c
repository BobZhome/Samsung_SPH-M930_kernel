...
static int s5k5aafa_sensor_init_probe(struct msm_camera_sensor_info *data)
{
	int rc = 0;
//	int read_value=-1;
...
int s5k5aafa_sensor_init(struct msm_camera_sensor_info *data)
{
	int rc = 0;
...
int s5k5aafa_sensor_ext_config(void __user *argp)
{
	...
	CAM_DEBUG("s5k5aafa_sensor_ext_config, cmd = %d ",cfg_data.cmd);
...
#endif
static int __init msm_vga_camera_probe(struct platform_device *pdev)
{
...
static int __init msm_vga_camera_init(void)
{
	return platform_driver_register(&msm_vga_camera_driver);
}

static void __exit msm_vga_camera_exit(void)
{
	platform_driver_unregister(&msm_vga_camera_driver);
}

//chief.boot.temp module_init(msm_vga_camera_init);
module_exit(msm_vga_camera_exit);
