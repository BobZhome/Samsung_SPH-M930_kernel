		...
		int ptype, int mixer, int req_share)
{
	...
	for (i = 0; i < MDP4_MAX_PIPE; i++) {
		...
		if(platform_reset_count && 
				...
				pipe->pipe_num == 2) {
			force_free_mdp_video_overlay_pipe = platform_reset_count;
			pr_err("!![%s] Freeup dangling pipe and try to restore!!!!\n", __func__);
...
int mdp4_overlay_change_z_order_vg_pipes(struct fb_info *info)
{
	...
	struct mdp4_overlay_pipe *pipeS1,*pipeS2;
        int stage1, stage2, mixer1, mixer2, pnum1, pnum2;

        pipeS1 = &ctrl->plist[4];       /* ndx start from 1 */
        pipeS2 = &ctrl->plist[5];       /* ndx start from 1 */

        stage1 = pipeS1->mixer_stage;
        mixer1 = pipeS1->mixer_num;
        pnum1 = pipeS1->pipe_num;

        stage2 = pipeS2->mixer_stage;
        mixer2 = pipeS2->mixer_num;
        pnum2 = pipeS2->pipe_num;
        printk(KERN_ERR "pipe 1 0x%x pipe 2 0x%x \n", (unsigned int)ctrl->stage[mixer1][stage1], (unsigned int)ctrl->stage[mixer2][stage2]);
        if(ctrl->stage[mixer1][stage1] == NULL || ctrl->stage[mixer2][stage2] == NULL)
	{
		mfd->z_order_change = 0;
		printk(KERN_ERR "mdp4_overlay_change_z_order_vg_pipes: less than two VG pipes are active \n");
		return -ENODEV;
	} else
	{
		mfd->z_order_change = 1;
		return 0;
	}

}

static int mdp4_overlay_change_z_order_vg_pipes_actual(struct msm_fb_data_type *mfd, struct mdp4_overlay_pipe *pipe)
{
 	int data;
 	int pipe1=0, pipe2=0;
        int temp;
	...
	mfd->z_order_change = 0;
        ...
        mixer2 = pipeS2->mixer_num;
        pnum2 = pipeS2->pipe_num;
        printk(KERN_ERR "Naomi: pipe 1 0x%x pipe 2 0x%x \n", (unsigned int)ctrl->stage[mixer1][stage1], (unsigned int)ctrl->stage[mixer2][stage2]);
        ...
        if(ctrl->stage[mixer1][stage1] == NULL || ctrl->stage[mixer2][stage2] == NULL)
	{
 	...
 	printk(KERN_ERR "mdp4_overlay_change_z_order_vg_pipes\n");
 	data = inpdw(MDP_BASE + 0x10100);
    printk(KERN_ERR "mdp4_overlay_change_z_order_vg_pipes data = 0x%x\n", data);
    pipe1 = (data & 0xf) << 4;
    printk(KERN_ERR "mdp4_overlay_change_z_order_vg_pipes pipe1 = 0x%x\n",pipe1);
    pipe2 = (data & 0xf0) >> 4;
    printk(KERN_ERR "mdp4_overlay_change_z_order_vg_pipes pipe2 = 0x%x\n", pipe2);
 	data &= 0xffffff00;
 	data |= pipe1;
 	data |= pipe2;
    printk(KERN_ERR "mdp4_overlay_change_z_order_vg_pipes data = 0x%x\n", data);
...
//	mdp4_mixer_stage_up(pipe);
	if (mfd->z_order_change == 1) {
		mdp4_overlay_change_z_order_vg_pipes_actual(mfd, pipe);
	} else
	{
		mdp4_mixer_stage_up(pipe);
	}
