...
#ifdef CONFIG_FB_MSM_SEC_BOOTLOGO
	if ( charging_boot ) {
		if (!load_565rle_image_onfb( "charging.rle",0,0)) ;
	}
#ifdef CONFIG_MACH_CHIEF
	else {
			if (!load_565rle_image_onfb( "SPH-D600.rle",0,0)) ;
	}
#elif CONFIG_MACH_VITAL2_BST
	else {
			...
			unsigned long arg)
{
	...
	case MSMFB_OVERLAY_CHANGE_ZORDER_VG_PIPES:
		down(&msm_fb_ioctl_ppp_sem);
		ret = msmfb_overlay_change_z_order_vg_pipes(info, argp);
		up(&msm_fb_ioctl_ppp_sem);
		break;
#endif
	case MSMFB_BLIT:
