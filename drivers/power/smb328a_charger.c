...
static void smb328a_changer_function_conrol(struct i2c_client *client)
{
	...
	if (val >= 0) {
		...
		if (data != 0x77) { /* this can be changed with top-off setting */
            /*  100x xxxx : Fast Charge Current = 900mA, --> 011 = 800mA
                xxx1 0xxx : Pre-Charge Current = 100mA,
                xxxx x111 : Charge Completion Termination Current = 200mA */
			data = 0x77;
	...
	if (val >= 0) {
		...
		if (data != 0x90) {
            /* 100x xxxx : AC Input Current Limit = 900mA
	...
	if (val >= 0) {
		...
		if (data != 0xda) {
            /* 1xxx xxxx : Automatic Recharge = Diabled
               ...
               xx01 1xxx : Pre-Charge to Fast-charge Voltage Threshold = 2.6V
...
#if defined (CONFIG_TARGET_LOCALE_USA) || defined (CONFIG_MACH_ROOKIE2) || defined(CONFIG_MACH_PREVAIL2)
		if (data != 0x45) {
			data = 0x45;
#else
...
static int smb328a_set_charging_current(struct i2c_client *client, int chg_current)
{
	...
	if (val >= 0) {
		input = (u8)val;
		printk("[SMB328a][%s] input before shifting: 0x%x\n", __func__, input);
		input &= ~(0x7 << 5);
		printk("[SMB328a][%s] input before shifting: 0x%x\n", __func__, input);
	} else {
	...
	if (val >= 0) {
		output = (u8)val;
		printk("[SMB328a][%s] output before shifting: 0x%x\n", __func__, output);
		output &= ~(0x7 << 5);
		printk("[SMB328a][%s] output after shifting: 0x%x\n", __func__, output);
	} else {
		printk("%s: can't read register(0x%x)\n",
	...
	if (val >= 0) {
		ac_usb = (u8)val;
		printk("[SMB328a][%s] ac_usb before shifting: 0x%x\n", __func__, ac_usb);
		ac_usb &= ~(0x1 << 2);
		printk("[SMB328a][%s] ac_usb after shifting: 0x%x\n", __func__, ac_usb);
	} else {
		printk("%s: can't read register(0x%x)\n",
			__func__, SMB328A_COMMAND);
		return -1;
	}
#if defined(CONFIG_MACH_ESCAPE) || defined (CONFIG_MACH_ROOKIE2) || defined(CONFIG_MACH_PREVAIL2)	//test:ashu
	if (chg_current == 450) {	
		input |= (0x0 << 5); /* 450mA disable, at auto-limit case */
		output |= (0x0 << 5); /* 500mA */
	#if defined(CONFIG_MACH_PREVAIL2)
		ac_usb |= (0x1 << 3); //AC Mode
	#else
...
#if defined(CONFIG_MACH_PREVAIL2)
	else if (chg_current == 800) {
		input |= (0x3 << 5); /* 800mA disable, at auto-limit case */
		output |= (0x3 << 5); /* 800mA */
		ac_usb |= (0x3 << 2);
#else
...
int smb328a_charger_control(int fast_charging_current, int termination_current, int on)
{
    ...
    if (!g_chip)
        {
		pr_info("[SMB328a] %s: device not found !!!\n", __func__);
		return -ENODEV ;
    	}
    ...
    if (on)
    {
		...
		pr_info("[BATT] %s: smb328a_disable_charging() ret_ = %d \n", __func__, ret);
    	}
			...
			const struct i2c_device_id *id)
{
   ...
   printk(KERN_ERR "[SMB328a][%s]\n", __func__);
	...
	{
//		printk(KERN_ERR "[SMB328a][%s]i2c_check_functionality failed.. Exiting unregistered\n", __func__);
		return -EIO;		
	}

	printk("%s: SMB328A driver Loading! \n", __func__);
	printk("%s: before kzalloc() : chip = %X\n", __func__, chip);

	chip = kzalloc(sizeof(*chip), GFP_KERNEL);
	printk("%s: after kzalloc() : chip = %X\n", __func__, chip);
	if (!chip)
		return -ENOMEM;
	...
	chip->chg_mode = CHG_MODE_NONE;
	//smb328a_changer_function_conrol(client);
...
#if defined(CONFIG_MACH_ESCAPE)
    if (hw_version < 2){
//		printk(KERN_ERR "[SMB328a][%s]: hw_version < 2 , exiting...\n", __func__);
        return -ENODEV ;
    	}
#endif
