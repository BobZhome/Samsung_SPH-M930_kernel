...
struct file *klib_fopen(
		...
		int flags,	/*
					...
					* O_NONBLOCK, O_SYNC,
					*/
		int mode	/* !< file creation permisstion. S_IRxxx S_IWxxx
					* S_IXxxx (xxx=USR,GRP,OTH),
					* S_IRWXx(x=U,G,O)
					*/
		)
{
		...
		)
{
	...
	if (filp == NULL) {
		printk(KERN_DEBUG "  filp == NULL\n");
		return -ENOENT;
	}

	if (filp->f_op->read == NULL) {
		printk(KERN_DEBUG "  filp->f_op->read == NULL\n");
		return -ENOSYS;
	}

	if (((filp->f_flags & O_ACCMODE) & O_RDONLY) != 0) {
		printk(KERN_DEBUG "  ((filp->f_flags & O_ACCMODE) & O_RDONLY) != 0\n");
		return -EACCES;
	}
		...
		)
{
	...
	if (filp->f_op->write == NULL) {
		printk(KERN_DEBUG	"  filp->f_op->write == NULL\n");
		return -ENOSYS;
	}
