...
unsigned param_start_page = 0;
unsigned param_end_page = 0;

static unsigned CFG0_M, CFG1_M, ECC_CFG_M;
static unsigned CFG0_A, CFG1_A, ECC_CFG_A;

unsigned boot_param_start_block;
#endif
...
static struct flash_identification supported_flash[] =
{
	...
	{0x6600b3ec, 0xFF00FFFF,(1024<<20), 1, 4096, (4096<<6), 128,}, /*Sams*/ //chief.boot
...
uint32_t flash_onfi_probe(struct msm_nand_chip *chip)
{
	...
	for ( i = 0 ; i < msm_nand_data.nr_parts ; i++) {
		if (!strcmp(msm_nand_data.parts[i].name , "parameter")) {
			...
			param_start_page = ((msm_nand_data.parts[i].offset) * 64 * 4096);
			param_end_page = (((msm_nand_data.parts[i].offset + 5) * 64 * 4096) - 1);
		}
...
void msm_read_param(struct samsung_parameter *param_data)
{
	...
	memcpy(param_data,data_buf,sizeof(struct samsung_parameter));
}
...
void msm_read_boot_param(struct BOOT_PARAM *param_data)
{
	...
	ret =  msm_nand_read_oob(current_mtd, ((boot_param_start_block) * 64)*4096, &ops);
	memcpy(param_data,data_buf,sizeof(struct BOOT_PARAM));
}

EXPORT_SYMBOL(msm_read_boot_param);
...
void msm_write_param(struct samsung_parameter *param_data)
{
	...
	memcpy(data_buf,param_data,sizeof(struct samsung_parameter));
...
void msm_write_boot_param(struct BOOT_PARAM *param_data)
{
	...
	param_erase_info = kzalloc(sizeof(struct erase_info), GFP_KERNEL);
	param_erase_info->mtd = current_mtd;
	param_erase_info->addr = (boot_param_start_block) * 64 * 4096;
	param_erase_info->len = current_mtd->erasesize;
	if(!msm_nand_erase(current_mtd, param_erase_info)) {
		pr_info("parameter block erase success\n");
	}
	...
	memset(data_buf,0x0,4096);
	memset(spare_buf,0xFF,128);
	memcpy(data_buf,param_data,sizeof(struct BOOT_PARAM));
	...
	ret =  msm_nand_write_oob(current_mtd, ((boot_param_start_block) * 64)*4096, &ops);
	
	kfree(param_erase_info);
}

EXPORT_SYMBOL(msm_write_boot_param);
