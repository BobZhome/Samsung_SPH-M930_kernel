...
struct usb_mode_func {
 ...
 struct usb_mode_func android_funcs[] = {
    { "rndis", USB_MODE_RNDIS },
    { "adb", USB_MODE_ADB },
    { "usb_mass_storage", USB_MODE_UMS },
    { "usb_mtp_gadget", USB_MODE_MTP },
    { "diag", USB_MODE_DIAG },
    { "modem", USB_MODE_MODEM },
    { "nmea", USB_MODE_NMEA },
    { "acm", USB_MODE_ACM },
    { "rmnet", USB_MODE_RMNET },
};

static int func_to_usb_mode(const char *func_name) {
    int i, size = ARRAY_SIZE(android_funcs);
    for (i=0; i<size; i++) {
        if (!strcmp(func_name, android_funcs[i].name)) {
            return android_funcs[i].mode;
        }
    }
    return 0;
}
