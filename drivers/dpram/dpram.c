...
enum {
	DPRAM_DEBUG_INIT = 1U << 0,
	DPRAM_DEBUG_GENERAL = 1U << 1,

	DPRAM_DEBUG_READ_SHORT = 1U << 4,
	DPRAM_DEBUG_READ = 1U << 5,
	DPRAM_DEBUG_READ_RAW_HEADER = 1U << 6,

	DPRAM_DEBUG_WRITE_SHORT = 1U << 8,
	DPRAM_DEBUG_WRITE = 1U << 9,

	DPRAM_DEBUG_RECV_IRQ = 1U << 12,
	DPRAM_DEBUG_SEND_IRQ = 1U << 13,

	DPRAM_DEBUG_SMEM_DBGINFO = 1U << 16,
};

static int dpram_debug_mask = DPRAM_DEBUG_INIT;
module_param_named(
	debug_mask, dpram_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
...
#define DPRAM_DPRINTK(mask, level, msg, ...) \
	do { \
		if ((mask) & dpram_debug_mask) \
			printk(level "[DPRAM] " msg, ## __VA_ARGS__); \
	} while (0)

#define __SMEM_DPRAM_DEBUG__
#ifdef __SMEM_DPRAM_DEBUG__
static samsung_vendor1_id *p_smem_vendor1;
#endif
...
#ifdef __SMEM_DPRAM_DEBUG__
#define MSM_TRIG_A2M_DPRAM_INT	do { \
									(writel(1 << 4, MSM_GCC_BASE + 0x8)); \
   									if(p_smem_vendor1) \
										p_smem_vendor1->dpram_debug.a2m.sent++; \
								} while(0)
#else
...
void check_miss_interrupt(void);
#ifdef CONFIG_DPRAM_EXTCMD
extern int cmd_extcmd_handler(void);
extern int dpram_register_extcmd_dev(void);
#endif
...
static spinlock_t write_spinlock;
...
static void emmc_read_param(struct work_struct *work);
static void emmc_write_param(struct work_struct *work);
extern int emmc_kernel_param_read(struct samsung_parameter *param_data);
extern int emmc_kernel_param_write(struct samsung_parameter *param_data);
#endif
...
static inline void byte_align(unsigned long dest, unsigned long src)
{
	...
	if (!(dest % 2) && !(src % 2)) {
		...
		*p_dest = (*p_dest & 0x00FF) | ((*p_src << 8) & 0xFF00);
		...
		DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_ERR, "oops.~\n");
	}
}
...
{
	DPRAM_DPRINTK(DPRAM_DEBUG_SEND_IRQ, KERN_INFO, "=====> send IRQ: %x \n", irq_mask);

	WRITE_TO_DPRAM(DPRAM_PDA2PHONE_INTERRUPT_ADDRESS, &irq_mask,
			DPRAM_INTERRUPT_PORT_SIZE);
	// hsil
	MSM_TRIG_A2M_DPRAM_INT;
//	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "PDA -> Phone interrupt!\n");
}
...
#define CLUSTER_SEGMENT	1500
	...
	if (dpram_debug_mask&DPRAM_DEBUG_READ) {
		int i;
	...
	if (head != tail) {
		...
		if (head > tail) {
			...
			if(size!= retval)
				...
				DPRAM_DPRINTK(DPRAM_DEBUG_READ_SHORT, KERN_INFO, "READ -return: %d\n", retval);
		}
		...
		// +++++++ head ------------ tail ++++++++ //
			...
			if (size > tmp_size) {
				...
				if((size - tmp_size)!= retval_add)
					...
					DPRAM_DPRINTK(DPRAM_DEBUG_READ_SHORT, KERN_INFO, "READ -return_add: %d\n", retval_add);
			}
		}
...
static int dpram_read_raw(dpram_device_t *device, const u16 non_cmd)
{
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_READ, KERN_INFO, "enter :dpram_read_raw\n");
...
//		printk(KERN_ERR "=====> %s,  head: %d, tail: %d, size: %d\n", __func__, head, tail, size);
		...
		while(size){
            ...
            DPRAM_DPRINTK(DPRAM_DEBUG_READ, KERN_INFO, "%02x ",ch);

			if(ch == 0x7f) {
				read_offset ++;
			}
			else {
				printk(KERN_ERR "[DPRAM] %s failed.. First byte: %d, drop byte: %d\n", __func__, ch, size);
			...
			DPRAM_DPRINTK(DPRAM_DEBUG_READ_RAW_HEADER, KERN_ERR, "[DPRAM] dpram_read_raw header info : len(%d) id(%d) control(%d)\n", hdr.len, hdr.id, hdr.control);

			if(len <= 0) {
				printk(KERN_ERR "[DPRAM] %s uups..\n", __func__);
...
//			printk(KERN_ERR "%s, %d read_offset: %d, len: %d hdr.id: %d\n", __func__, __LINE__, read_offset, len, hdr.id);
			...
			if(!dev) {
				printk(KERN_ERR "[DPRAM] %s failed.. NULL dev detected \n", __func__);
				dpram_drop_data(device);
				return -1;
			}
...
//					printk(KERN_ERR "=====> pre_data_size: %d, len-pre_data_size: %d, ret: %d\n", pre_data_size, len- pre_data_size, ret);
			...
			if(!ret) {
				printk(KERN_ERR "[DPRAM] %s failed.. (tty_insert_flip_string) drop byte: %d\n", __func__, size);
				printk(KERN_ERR "buff addr: %lx\n", (device->in_buff_addr));
				printk(KERN_ERR "read addr: %lx\n", (device->in_buff_addr + ((u16)(tail + read_offset) % device->in_buff_size)));
				dpram_drop_data(device);
				return -1;
...
//			printk(KERN_ERR "%s,%d read_offset: %d ret= %d\n", __func__, __LINE__, read_offset, ret);
			...
			if(ch == 0x7e)
				...
				printk(KERN_ERR "[DPRAM] %s failed.. Last byte: %d, drop byte: %d\n", __func__, ch, size);
				printk(KERN_ERR "buff addr: %lx\n", (device->in_buff_addr));
				printk(KERN_ERR "read addr: %lx\n", (device->in_buff_addr + ((u16)(tail + read_offset) % device->in_buff_size)));
				dpram_drop_data(device);
				return -1;
...
//			printk(KERN_ERR "%s, %d retval= %d, read_offset: %d, size: %d\n", __func__, __LINE__, retval, read_offset, size);
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_READ, KERN_INFO, "exit :dpram_read_raw\n");
...
static void dpram_clear(void)
{
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "SmemBase = %x\n", (unsigned int)SmemBase);
...
//	const u16 init_start = INT_COMMAND(INT_MASK_CMD_INIT_START);
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "start\n");
...
//	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "DPRAM_MAiGIC_CODE_ADDRESS value = 0x%x\n", value);
//	READ_FROM_DPRAM(&value, DPRAM_ACCESS_ENABLE_ADDRESS, sizeof(value));
//	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "DPRAM_ACCESS_ENABLE_ADDRESS value = %d\n", value);
	...
	//hsil
	MSM_TRIG_A2M_DPRAM_INT;
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "finish\n");
}
...
static inline int dpram_get_read_available(dpram_device_t *device)
{
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "%s : head = 0x%x\n", __func__, head);
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "%s : tail = 0x%x\n", __func__, tail);
	return head - tail;
}
...
// hsil
#ifdef MSM7201A
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "\n");
...
//	const u16 reboot_magic_code = 0x00ab;
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "[RAM DUMP] REBOOT_MAGIC_CODE\n");

	READ_FROM_DPRAM((void *)&magic_read, DPRAM_MAGIC_CODE_ADDRESS, sizeof(magic_read));
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "[RAM DUMP] Prev Magic Code : 0x%x\n", magic_read);

	WRITE_TO_DPRAM(DPRAM_MAGIC_CODE_ADDRESS, &reboot_magic_code, sizeof(reboot_magic_code));
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "[RAM DUMP] SMSM WRITE\n");

	READ_FROM_DPRAM((void *)&magic_read, DPRAM_MAGIC_CODE_ADDRESS, sizeof(magic_read));
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "[RAM DUMP] Cur Magic Code : 0x%x\n", magic_read);
...
static int dpram_tty_open(struct tty_struct *tty, struct file *file)
{
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "tty->index = %d\n", tty->index);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case HN_DPRAM_PHONE_ON:
			if (DpramInited)
			{
				DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "Doubled Phone On Cmd : do nothing\n");
				return 0;
			}
			DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "[Version 3] HN_DPRAM_PHONE_ON\n");
			dpram_phone_on();
			DpramInited = 1;
			return 0;

		case HN_DPRAM_PHONE_OFF:
			DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "HN_DPRAM_PHONE_OFF\n");
			dpram_phone_off();
			return 0;

		case HN_DPRAM_PHONE_GETSTATUS:
			DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "HN_DPRAM_PHONE_GETSTATUS\n");
			val = dpram_phone_getstatus();
			return copy_to_user((unsigned int *)arg, &val, sizeof(val));

		case HN_DPRAM_PHONE_RESET:
			DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "[RAM DUMP]HN_DPRAM_PHONE_RESET\n");
			dpram_phone_reset();
			return 0;
		...
		case HN_DPRAM_MEM_RW:
		{
			...
			DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "HN_DPRAM_MEM_RW\n");
...
*/
		...
		default:
			DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "default\n");
			break;
	}
...
static void fmt_rcv_tasklet_handler(unsigned long data)
{
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "%s\n", __func__);
	if (device && device->serial.tty) {
		struct tty_struct *tty = device->serial.tty;
		...
		while (dpram_get_read_available(device)) {
			...
			if (ret < 0) {
				DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "%s : dpram_read fail\n", __func__);
				/* TODO: ... wrong.. */
			}
		...
		DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "%s : will drop data\n", __func__);
		dpram_drop_data(device);
	}
...
static void cmd_error_display_handler(void)
{
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "[PHONE ERROR] ->> %s\n", buf);
...
//chief.temp	get_charger_type();
//chief.temp	get_charging_status();
//chief.temp	msm_batt_check_event();
...
#ifdef CONFIG_DPRAM_EXTCMD
		case INT_MASK_CMD_EXTENDED_CMD:
			cmd_extcmd_handler();
			break;
#endif

		default:
			DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "Unknown command (%x)..\n", cmd);
	}
}
...
static DECLARE_TASKLET(dpram_fake_irq_tasklet, dpram_fake_irq_handler, 0);

int dpram_exit_sleep(bool from_idle, int wakeup_reason)
{
	if(from_idle && wakeup_reason&0x100) {
		void dpram_irq_clear(void);
		//dpram_irq_clear();
		tasklet_hi_schedule(&dpram_fake_irq_tasklet);
	}
...
//	u16 reset_int = 0;
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "%s : interrupt handler\n", __func__);

#ifdef __SMEM_DPRAM_DEBUG__
   if(p_smem_vendor1) p_smem_vendor1->dpram_debug.m2a.rcvd++;

   DPRAM_DPRINTK(DPRAM_DEBUG_SMEM_DBGINFO, KERN_INFO, " m2a[sent:%10d][rcvd:%10d]\n[DPRAM] a2m[sent:%10d][rcvd:%10d]\n",
   						p_smem_vendor1->dpram_debug.m2a.sent,
						p_smem_vendor1->dpram_debug.m2a.rcvd,
						p_smem_vendor1->dpram_debug.a2m.sent,
						p_smem_vendor1->dpram_debug.a2m.rcvd);
#endif

	DPRAM_DPRINTK(DPRAM_DEBUG_RECV_IRQ, KERN_ERR, "<===== received dpram interrupt\n");
...
//	check_int_pin_level();	// hsil
	...
	if (!(irq_mask & INT_MASK_VALID)) {
		DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "Invalid interrupt mask: 0x%04x\n", irq_mask);
		return IRQ_NONE;
	}
	...
	if (irq_mask & INT_MASK_COMMAND) {
		...
		DPRAM_DPRINTK(DPRAM_DEBUG_RECV_IRQ, KERN_ERR, "<===== received command IRQ: %x\n", irq_mask);
                ...
                disable_irq_nosync(INT_A9_M2A_4);
              #endif
	}

	else {
		irq_mask &= ~INT_MASK_VALID;
		DPRAM_DPRINTK(DPRAM_DEBUG_RECV_IRQ, KERN_ERR, "<===== received NON command IRQ: %x\n", irq_mask);
...
//	dpram_tty_driver->num = 1;
	...
	if (retval) {
		DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "tty_register_driver error\n");
...
static int register_interrupt_handler(void)
{
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "Dpram clear start\n");
	dpram_clear();

	retval = request_irq(INT_A9_M2A_4, dpram_interrupt, IRQF_TRIGGER_RISING, DRIVER_NAME, NULL);

	if (retval) {
		DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "DPRAM interrupt handler failed.\n");
		unregister_dpram_driver();
		return -1;
	}
	DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "INT_A9_M2A_4 interrupt handler success\n");
 ...
 void check_miss_interrupt(void)
{
	...
	if (head != tail)
	{
		DPRAM_DPRINTK(DPRAM_DEBUG_GENERAL, KERN_INFO, "there is a missed interrupt. try to read it!\n");
...
static int __devinit dpram_probe(struct platform_device *dev)
{
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_INIT, KERN_INFO, "SMEM_DPRAM allocation\n");
...
//	printk("=====>[%s,%d] SMEM vendor0 id: %d\n", __func__, __LINE__, SMEM_ID_VENDOR0);
	if (!SmemBase)
	{
		DPRAM_DPRINTK(DPRAM_DEBUG_INIT, KERN_INFO, "smem_alloc failed : SmemBase = 0x%x\n", (unsigned int)SmemBase);
		return -1;
	}
   ...
   INIT_WORK(&dpram_irq_work, dpram_irq_work_queue);
	...
	if (retval) {
		DPRAM_DPRINTK(DPRAM_DEBUG_INIT, KERN_INFO, "Failed to register dpram (tty) driver.\n");
		return -1;
	}
	...
	if (retval) {
		DPRAM_DPRINTK(DPRAM_DEBUG_INIT, KERN_INFO, "Failed to register dpram error device.\n");
	...
	if (retval) {
		DPRAM_DPRINTK(DPRAM_DEBUG_INIT, KERN_INFO, "Failed to register onedram device.\n");
...
//	enable_dpram_pins();	// hsil
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_INIT, KERN_INFO, "Register interrupt handler\n");
	...
	DPRAM_DPRINTK(DPRAM_DEBUG_INIT, KERN_INFO, DRIVER_ID "\n");
...
static void emmc_write_param(struct work_struct *work)
{
	...
	if(param_data.ram_dump_level !=  dump_enable){
		param_data.ram_dump_level = dump_enable;
		emmc_kernel_param_write(&param_data);

		printk("%s nandparm->ram_dump_level : %d\n",__func__ ,dump_enable);
	}
}
static void emmc_read_param(struct work_struct *work)
{
	...
	if((param_data.ram_dump_level_init != 0x12345676) || (param_data.ram_dump_level > 2)){
			param_data.ram_dump_level_init = 0x12345676;
			param_data.ram_dump_level = dump_enable;
			emmc_kernel_param_write(&param_data);

			printk("%s Set_ram_dump_level_init nandparm->ram_dump_level : %d\n",__func__ ,dump_enable);
		}
		                    ...
		                    int count, int *eof, void *data)
{
	*eof = 1;
	...
	{
		msleep(6000);
		msm_proc_comm(PCOM_RESET_CHIP, &multi_download_mode, 0);
	}
	test ++;
	#endif
		                    ...
		                    unsigned long count, void *data)
{
	...
	if (buf[0] == '0') {               //low
		...
		printk("Set silent : %d\n", dump_enable);
			...
			#else
				dump_enable = 2;
				queue_work(emmc_param_work_queue,&emmc_param_read_access);
			#endif
	...
	#else  //VITAL2
		queue_work(emmc_param_work_queue,&emmc_param_write_access);
	#endif
	kfree(buf);
...
static int multi_download_read_proc_debug(char *page, char **start, off_t offset,
		                    int count, int *eof, void *data)
{
	...
	char string[20]="DOWNLOAD";
		...
		if ( !(boot_param_data = kzalloc(sizeof(struct BOOT_PARAM),GFP_KERNEL))) {
			...
			memset(boot_param_data,0,sizeof(struct BOOT_PARAM));
			msm_read_boot_param(boot_param_data);
			memcpy(string,boot_param_data->multi_download,sizeof(string));
			kfree(boot_param_data);
		}
	#else
...
static int multi_download_write_proc_debug(struct file *file, const char *buffer,
		                    unsigned long count, void *data)
{
	char *buf;
	struct BOOT_PARAM *boot_param_data;
	...
	if (copy_from_user(buf, buffer, count)) {
	    ...
	    //to write dump_level to parm
		if ( !(boot_param_data = kzalloc(sizeof(struct BOOT_PARAM),GFP_KERNEL))) {
			printk("######### can not alloc memory for write boot_param_data ! ##################\n");
		}
		else{
			memset(boot_param_data,0,sizeof(struct BOOT_PARAM));
			msm_read_boot_param(boot_param_data);
			memcpy(&(boot_param_data->multi_download),buf,count);
			msm_write_boot_param(boot_param_data);
			kfree(boot_param_data);
		}
	#else
		if(!strncmp(buf,"DOWNLOAD",8))
			multi_download_mode = 0xabcd1234;
...
/* init & cleanup. */
struct proc_dir_entry *ram_dump_level_ent;
...
//	printk("smem_silent_mode : 0x%08x\n", smem_silent_mode->info);
	...
	ent = create_proc_entry("multi_download", S_IRWXU|S_IRGRP|S_IROTH, NULL);
	ent->read_proc = multi_download_read_proc_debug;
	ent->write_proc = multi_download_write_proc_debug;
error_return:
...
static int vs_open(struct tty_struct *tty, struct file *filp)
{
	...
	printk(KERN_ERR "[%s] name:%s current:%s\n", __func__, tty->driver->name, current->comm);
...
static int vs_ioctl(struct tty_struct *tty, struct file *file,
		    unsigned int cmd, unsigned long arg)
{
	printk(KERN_ERR "[%s] name:%s cmd:%x\n", __func__, tty->driver->name, cmd);
	return -ENOIOCTLCMD;
}
