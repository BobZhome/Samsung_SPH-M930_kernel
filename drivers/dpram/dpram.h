...
#if defined(CONFIG_DPRAM_EXTCMD)
#define INT_MASK_CMD_EXTENDED_CMD		0x000E
#endif
...
struct extcmd_hdr {
	...
	unsigned int subcmd;
	int args[5];
	volatile int status; /* Write 0 when sending command. And wait for it to become a non-zero value */
};

/* This is the data stored in the smem (SMEM_ID_VENDOR2) */
struct extcmd {
	int semaphore; /* 0: free, 1:owned by AP, 2:owned by CP */
	int reserved[3];
	struct extcmd_hdr hdr;
	char data[32*1024]; /* 32 KB */
};

struct extcmd_rx_handler_data {
	unsigned int cmd;
	int (*handler)(unsigned int subcmd, int* args, void* data);
};

struct extcmd_tx_data {
	struct extcmd_hdr hdr;
	int *data;
};

struct extcmd_dev {
	volatile struct extcmd *extcmd; /* this is the actual SMEM data */
	struct extcmd_rx_handler_data *rx_handler_data;
	int num_rx_handlers; 
	struct extcmd_tx_data tx_data;
