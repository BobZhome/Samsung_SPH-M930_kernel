...
static DEVICE_ATTR(hp_gainup, S_IRUGO|S_IWUSR, hp_gainup_show, 	hp_gainup_store);
static DEVICE_ATTR(sp_att	, S_IRUGO|S_IWUSR, sp_att_show, 	sp_att_store);
static DEVICE_ATTR(sp_gainup, S_IRUGO|S_IWUSR, sp_gainup_show, 	sp_gainup_store);
...
void yda165_speaker_onoff_incall(int onoff) /* speaker path amp onoff */
{
	D4HP3_SETTING_INFO stInfo;
#if AMPREG_DEBUG	
	unsigned char buf;
	...
	{		
		...
		/* input */
		stInfo.bLine1Gain = g_ampgain[1].in1_gain;		/* LINE1 Gain Amp */
		stInfo.bLine2Gain = g_ampgain[1].in2_gain;		/* LINE2 Gain Amp */
		...
		stInfo.bHpAvddLev = 0;			/* HP charge pump AVDD level, 1.65V<=AVDD<2.40V(0) / 2.40V<=AVDD<=2.86V(1) */
		stInfo.bHpEco = 0;				/* HP eco mode, normal(0) / eco mode(1) */
		stInfo.bHpAtt = g_ampgain[1].hp_att;				/* HP attenuator */
		stInfo.bHpGainUp = g_ampgain[1].hp_gainup;			/* HP gain up */
		stInfo.bHpSvol = 0;				/* HP soft volume setting, on(0) / off(1) */
		stInfo.bHpZcs = 1;;				/* HP zero cross mute setting, on(0) / off(1) */
		stInfo.bHpCh = 0;				/* HP channel, stereo(0)/mono(1) */
		stInfo.bHpMixer_Line1 = 0;		/* HP mixer LINE1 setting */
		...
		/* SP */
		stInfo.bSpAtt = g_ampgain[1].sp_att;				/* SP attenuator */
		stInfo.bSpGainUp = g_ampgain[1].sp_gainup;			/* SP gain up */
		stInfo.bSpSvol = 0;				/* SP soft volume setting, on(0) / off(1) */
		stInfo.bSpZcs = 0;				/* SP zero cross mute setting, on(0) / off(1) */
		stInfo.bSpMixer_Line1 = 1;		/* SP mixer LINE1 setting */
		stInfo.bSpMixer_Line2 = 0;		/* SP mixer LINE2 setting */
		stInfo.bSpNg_DetectionLv = 1;	/* SP Noise Gate : detection level */
		stInfo.bSpNg_AttackTime = 1;		/* SP Noise Gate : attack time */
		stInfo.bSpNcpl_NonClipRatio = 0;	/* SP Non-Clip power limiter : Non-Clip distortion ratio */
		stInfo.bSpNcpl_PowerLimit = 0;	/* SP Non-Clip power limiter : Power Limit */
		stInfo.bSpNcpl_AttackTime = 2;	/* SP Non-Clip power limiter : attack Time */
...
void yda165_speaker_onoff_inVoip(int onoff) /* speaker path amp onoff */
{
	D4HP3_SETTING_INFO stInfo;
#if AMPREG_DEBUG	
	unsigned char buf;
	...
	if (onoff)
	{
		...
		/* input */
		stInfo.bLine1Gain = g_ampgain[0].in1_gain;		/* LINE1 Gain Amp */
		stInfo.bLine2Gain = g_ampgain[0].in2_gain;		/* LINE2 Gain Amp */
		...
		stInfo.bHpAvddLev = 0;			/* HP charge pump AVDD level, 1.65V<=AVDD<2.40V(0) / 2.40V<=AVDD<=2.86V(1) */
		stInfo.bHpEco = 0;				/* HP eco mode, normal(0) / eco mode(1) */
		stInfo.bHpAtt = g_ampgain[0].hp_att;				/* HP attenuator */
		stInfo.bHpGainUp = g_ampgain[0].hp_gainup;			/* HP gain up */
		stInfo.bHpSvol = 0;				/* HP soft volume setting, on(0) / off(1) */
		stInfo.bHpZcs = 1;;				/* HP zero cross mute setting, on(0) / off(1) */
		stInfo.bHpCh = 0;				/* HP channel, stereo(0)/mono(1) */
		stInfo.bHpMixer_Line1 = 0;		/* HP mixer LINE1 setting */
		...
		/* SP */
		stInfo.bSpAtt = g_ampgain[0].sp_att;				/* SP attenuator */
		stInfo.bSpGainUp = g_ampgain[0].sp_gainup;			/* SP gain up */
		stInfo.bSpSvol = 0;				/* SP soft volume setting, on(0) / off(1) */
		stInfo.bSpZcs = 0;				/* SP zero cross mute setting, on(0) / off(1) */
		stInfo.bSpMixer_Line1 = 1;		/* SP mixer LINE1 setting */
		stInfo.bSpMixer_Line2 = 0;		/* SP mixer LINE2 setting */
		...
		stInfo.bSpNg_AttackTime = 1;		/* SP Noise Gate : attack time */
		stInfo.bSpNcpl_NonClipRatio = 0;	/* SP Non-Clip power limiter : Non-Clip distortion ratio */
		stInfo.bSpNcpl_PowerLimit = 11;	/* SP Non-Clip power limiter : Power Limit */
		stInfo.bSpNcpl_AttackTime = 2;	/* SP Non-Clip power limiter : attack Time */
		stInfo.bSpNcpl_ReleaseTime = 1;	/* SP Non-Clip power limiter : release Time */
