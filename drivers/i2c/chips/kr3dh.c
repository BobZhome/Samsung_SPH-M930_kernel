...
static char kr3dh_write(struct i2c_client *client, unsigned char reg_addr, unsigned char *data, unsigned char len)
{
	...
	{ 
		...
		if(dummy)
		{
			dev_info(&client->dev,"i2c write error\n");
			return dummy; 
		}
	}
	return 0;
...
static char kr3dh_read(struct i2c_client *client, unsigned char reg_addr, unsigned char *data, unsigned char len) 
{
	int dummy=0;
	int i=0;
	//printk(KERN_INFO"%s\n", __FUNCTION__);
	...
	{        
		...
		{
			dev_info(&client->dev," i2c read error\n "); 
			return dummy;
		}
		dummy = len;
	}
...
static void kr3dh_set_delay(struct device *dev, unsigned long delay)
{
	...
	printk("%s actual_delay(delay)[%lu] kr3dh->odr[%d]\n",__func__,actual_delay(delay),kr3dh->odr);
		...
		const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev,"kr3dm_accel_driver probe start!!\n");
	...
	/* 1. enable vreg before i2c setting to fix a timing issue */
	if(kr3dh->pdata && kr3dh->pdata->vreg_en) {
		//extern int vreg_print_info(const char *what);
		//kr3dh->pdata->vreg_en(0);
		kr3dh->pdata->vreg_en(1);

		//vreg_print_info("enabled");
		//vreg_print_info("all");
	}
	...
	dev_info(&client->dev,"kr3dm_accel_driver probe complete!!\n");
