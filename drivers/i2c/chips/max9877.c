...
int max9877_i2c_tty_onoff(int nOnOff);
...
#define I2C_WRITE(reg,data) if (!max9877_i2c_write(reg, data) < 0) printk("max9877_i2c_write error\n");
#define I2C_READ(reg,data) if (max9877_i2c_read(reg,data) < 0 ) printk("max9877_i2c_read error\n");
...
int max9877_i2c_sensor_init(void)
{
	...
	I2C_WRITE(MAX9877_INPUTMODE_CTRL,0x40); // pre-gain 0dB //by interpark
...
int max9877_i2c_speaker_headset_onoff(int nOnOff)
{
		...
		if (nOnOff)
			printk("max9877 speaker_headset on \n");
		else
			printk("max9877 speaker_headset off \n");
...
//		is_incallMode = 0;
        ...
        I2C_WRITE(MAX9877_INPUTMODE_CTRL,0x40); // dINA = 0(stereo), dINB = 0(stereo) // by interpark
        if( nOnOff )
        {
            I2C_READ(MAX9877_OUTPUTMODE_CTRL,&reg_value);
...
int max9877_i2c_speaker_onoff(int nOnOff)
{
	...
	unsigned char reg_value2;
	unsigned char reg_value3;
	unsigned char reg_value4;
#endif
...
#if 0  // spk test by interpark
	I2C_READ(MAX9877_INPUTMODE_CTRL,&reg_value0);
	I2C_READ(MAX9877_SPKVOL_CTRL,&reg_value1); 
	I2C_READ(MAX9877_LEFT_HPHVOL_CTRL,&reg_value2); 
	I2C_READ(MAX9877_RIGHT_HPHVOL_CTRL,&reg_value3);
    I2C_READ(MAX9877_OUTPUTMODE_CTRL,&reg_value4);
	printk("[oops] INPUTMODE_CTRL = %d\n", reg_value0);
	printk("[oops] SPKVOL_CTRL = %d\n", reg_value1);
	printk("[oops] LEFT_HPHVOL_CTRL = %d\n", reg_value2);
	printk("[oops] RIGHT_HPHVOL_CTRL = %d\n", reg_value3);
	printk("[oops] OUTPUTMODE_CTRL = %d\n", reg_value4);
#endif
	return 0;
}
...
//		printk("%s(%d) - MIC_SEL=%d.\n", __func__, nOnOff, gpio_get_value(PATH_MIC_SEL));
		...
		if(is_incallMode)
		{
			...
			{
				//I2C_WRITE(MAX9877_INPUTMODE_CTRL,0x50); // dINA = 0(stereo), dINB = 1(mono)
				I2C_WRITE(MAX9877_INPUTMODE_CTRL,0x40); // dINA = 0(stereo), dINB = 1(stereo) // by interpark
			} else {
				I2C_WRITE(MAX9877_INPUTMODE_CTRL,0x70); // dINA = 1(mono), dINB = 1(mono)
			}
		} else {
...
int max9877_i2c_tty_onoff(int nOnOff)
{
		...
		if (nOnOff)
			printk("max9877 tty on \n");
		else
			printk("max9877 tty off \n");
...
//		printk("%s(%d) - MIC_SEL=%d.\n", __func__, nOnOff, gpio_get_value(PATH_MIC_SEL));
		...
		if(is_incallMode)
		{
			...
			{
				//I2C_WRITE(MAX9877_INPUTMODE_CTRL,0x50); // dINA = 0(stereo), dINB = 1(mono)
				I2C_WRITE(MAX9877_INPUTMODE_CTRL,0x70); // dINA = 0(stereo), dINB = 1(stereo) // by interpark
			} else {
				I2C_WRITE(MAX9877_INPUTMODE_CTRL,0x70); // dINA = 1(mono), dINB = 1(mono)
			}
		} else {
...
static int max9877_probe(struct i2c_client *client, const struct i2c_device_id * dev_id)
{
...
int max9877_legacy_suspend(void)
{
	int ret; 
	unsigned char reg_value;
...
int max9877_legacy_resume(void)
{
    int ret; 
    unsigned char reg_value;
...
EXPORT_SYMBOL(max9877_i2c_tty_onoff);
EXPORT_SYMBOL(max9877_legacy_suspend);
EXPORT_SYMBOL(max9877_legacy_resume);
