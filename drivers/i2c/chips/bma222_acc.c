...
#include <linux/i2c/bma222_acc.h>
...
{	
	...
	res = bma222_init( &bma222 );
	if(res != 0)
		return res;	

	printk("[%s] chip_id : %x\n",__func__,bma222.chip_id ); 	
	if ((bma222.chip_id & 0xFF) != BMA222_CHIP_ID)
	    return -ENODEV;
		
	printk("[%s] success to get chip ID\n", __func__);
	...
	res = bma222_set_range(BMA222_RANGE_2G); 
	if(res != 0)
		return res;
		
	/* set bandwidth to 25 HZ */
	//BMA222_BW_7_81HZ
	res = bma222_set_bandwidth(0);
	if(res != 0)
		return res;
...
//TEST
	...
	result = bma222_chip_init();
	if(result)
	{
		printk(KERN_ERR "[%s] bma222 chip init failed. err = %d\n", __func__, result);
		return result;
	}
