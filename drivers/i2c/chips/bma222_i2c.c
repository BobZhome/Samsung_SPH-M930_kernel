...
#include <linux/i2c/bma222_i2c.h>
...
int i2c_acc_bma222_read(u8 reg, u8 *val, unsigned int len )
{
	int 	 err;
	...
	*data       = reg;
	...
	printk("%s %d i2c transfer error. err = %d\n", __func__, __LINE__, err);/* add by inter.park */

	return err;

}
int i2c_acc_bma222_write( u8 reg, u8 *val )
{
	int err;
	struct i2c_msg msg[1];
