...
static ssize_t mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s", (current_mode==USB_3803_MODE_HUB)?"hub":"bypass");
}
...
int usb3803_set_mode(int mode)
{
	...
	switch(mode) {
		case USB_3803_MODE_HUB:
			...
			/* SMSC said CS version need to support Step1 & 2 only */
			int i;
			char data;
...
int usb3803_suspend(struct i2c_client *client, pm_message_t mesg)
{
	pdata->reset_n(0);
	pdata->clock_en(0);
	pr_info("USB3803 suspended with mode %s\n", (current_mode==USB_3803_MODE_HUB)?"hub":"bypass");
...
int usb3803_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("USB3803(%s version) probed on mode %s\n", (pdata->es_ver==1)?"ES":"CS", (current_mode==USB_3803_MODE_HUB)?"hub":"bypass");
