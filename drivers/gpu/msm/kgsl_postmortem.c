...
#if 1 //qc bug temporary fix
	// strncat does not limit total strlen, it limits additional string attached
	// therefore below codes can corrupt stack(when stack lb[90] was used)
	// so i'll just use kmalloc and allocate more than worst case string length
	lb = kzalloc(1024, GFP_KERNEL); // this should be more than enough...
	if(!lb) return;
#endif
